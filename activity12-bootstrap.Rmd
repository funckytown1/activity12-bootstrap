...
**Response**: THis graphs seems to have two primary peaks with one around 12 and one around 17. This makes sense as it would aline with a large group at the highschool leve l and a large group at the college achoelers degree level.
...
**Response**: No as it will be heavily influenced by outliers. Median would be a better value.
...
***Response**: It does not seem reasonable due to the multiple peaks and irregular shape of the graph
...
set.seed(000310)
boot_ed <- rerun(1000, gss_educ$educ) %>%
                map_dbl(mean)
```
...
```{r quantile}
quantile(boot_ed,probs=seq(.1,.90))
```


Compare this bootstrap interval to your `t.test` confidence interval from above.

I got the same value for both confidence intervals
